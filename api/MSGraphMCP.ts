...
export class MSGraphMCP {
  ...
  async initialize() {
    ...
    if (
      authConfig.mode === AuthMode.ClientProvidedToken &&
      this.authContext.accessToken
    ) {
      this.authManager.updateAccessToken(
        this.authContext.accessToken,
        undefined,
    ...
    server.tool(
      ...
      "A versatile tool to interact with Microsoft APIs including Microsoft Graph (Entra) and Azure Resource Management. IMPORTANT: For Graph API GET requests using advanced query parameters ($filter, $count, $search, $orderby), you are ADVISED to set 'consistencyLevel: \"eventual\"'.",
      {
        apiType: z
          .enum(["graph", "azure"])
          .describe(
            "Type of Microsoft API to query. Options: 'graph' for Microsoft Graph (Entra) or 'azure' for Azure Resource Management."
          ),
        path: z
          .string()
          .describe(
      ...
      async ({
        ...
        try {
          ...
          if (!fetchAll && method === "get") {
            const nextLinkKey =
              apiType === "graph" ? "@odata.nextLink" : "nextLink";
    ...
    server.tool(
      ...
      "Set or update the access token for Microsoft Graph authentication. Use this when the MCP Client has obtained a fresh token through interactive authentication.",
      {
        accessToken: z
          .string()
          .describe(
            "The access token obtained from Microsoft Graph authentication"
          ),
        refreshToken: z
          .string()
          .optional()
        ...
        expiresOn: z
          ...
          .optional()
          .describe(
            "Token expiration time in ISO format (optional, defaults to 1 hour from now)"
          ),
      },
      async ({ accessToken, refreshToken, expiresOn }) => {
        try {
          if (
            this.authManager?.getAuthMode() === AuthMode.ClientProvidedToken
          ) {
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          const tokenStatus = this.authManager
            ? await this.authManager.getTokenStatus()
            : { isExpired: false };
    ...
    server.tool(
      ...
      "Get applications from Microsoft Graph",
      {
        queryParams: z
          .record(z.string())
          .optional()
          .describe("Query parameters for filtering applications"),
        fetchAll: z
          .boolean()
          .optional()
          .default(false)
          .describe("Fetch all applications"),
      },
      async ({ queryParams, fetchAll }) => {
        const apps = await this.msGraphServiceInstance.getApplications(
          queryParams,
          fetchAll
        );
              ...
              if (body.method === "initialize") {
                        ...
                        tools: [
                          {
                            ...
                            description: "Chat with Microsoft Graph",
                          },
                        ],
                      },
                      serverInfo: {
                ...
                const tools = [
                  ...
                  },
                  {
                    ...
                    description:
                      "Set or update the access token for Microsoft Graph authentication",
                    inputSchema: {
                      type: "object",
                      properties: {
              ...
              JSON.stringify({
                ...
                result: {
                  content: [
                    {
                      type: "text",
                      text: "Tool execution over SSE not yet implemented",
                    },
                  ],
                },
              }),

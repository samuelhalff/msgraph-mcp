...
export class MSGraphMCP {
    ...
    private get msGraphServiceInstance(): MSGraphService {
        if (!this._msGraphService) {
            if (!this.authContext.accessToken) {
                // This is the clean error that allows LibreChat's auth flow to work.
                throw new Error("OAuth authentication required.");
            }
            // Create a minimal authConfig from env to satisfy MSGraphService constructor.
            // Adjust the fields as needed to match the AuthConfig shape in MSGraphService.ts.
            const authConfig = {
            ...
            this._msGraphService = new MSGraphService(this.env, this.authContext, authConfig);
        }
    ...
    get server() {
        ...
        server.tool('getUsers', 'Get users from Microsoft Graph', {
            ...
            fetchAll: z.boolean().optional().default(false).describe("Set to true to fetch all users")
        ...
        server.tool('getGroups', 'Get groups from Microsoft Graph', {
            ...
            fetchAll: z.boolean().optional().default(false).describe("Set to true to fetch all groups")
        }, async ({queryParams, fetchAll}) => {
        ...
        server.tool('createDraftEmail', 'Create an email draft in Outlook', {
             subject: z.string().optional(),
             body: z.string().optional(),
             // ... your full Zod schema for this tool
        }, async (params) => {
             const message = { subject: params.subject, body: { content: params.body, contentType: 'Text' } /* build full message object */ };
             const response = await this.msGraphServiceInstance.genericGraphRequest('/me/messages', 'post', message);
        ...
        return {
            fetch: async (request: Request) => {
                ...
                const authContext: MSGraphAuthContext = {
                    accessToken: authHeader?.startsWith('Bearer ') ? authHeader.substring(7) : '',
